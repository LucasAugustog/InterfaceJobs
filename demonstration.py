...
# ======================== CONEXOES ========================
#MySQL
conn_business = mysql.connector.connect(host="", user="", password="", database="")
...
def job_updateTabela_Integrado5(data_inicio=None, data_fim=None):
    ...
    sql = """select valor from parametro_integrador where descricao like 'Integrado5' """
...
def job_updateTabela_vendaCheckout(data_inicio=None, data_fim=None):
    ...
    conn_business2 = mysql.connector.connect(host="", user="", password="", database="")
    cursorSQLBusiness2 = conn_business2.cursor()

    #connection2 = oracledb.connect(user="", password="", dsn="")
    #cursorOracle2 = connection2.cursor()
        
    sql = """select valor from parametro_integrador where descricao like 'vendaCheckout' """
...
def job_updateTabela_vendaFormaPagamento(data_inicio=None, data_fim=None):
    ...
    conn_business3 = mysql.connector.connect(host="", user="", password="", database="")
    cursorSQLBusiness3 = conn_business3.cursor()

    connection3 = oracledb.connect(user="", password="", dsn="")
    cursorOracle3 = connection3.cursor()

    sql = """select valor from parametro_integrador where descricao like 'vendaFormaPagamento' """
...
def job_updateTabela_auditoriaPdv(data_inicio=None, data_fim=None):
    ...
    conn_business4 = mysql.connector.connect(host="", user="", password="", database="")
    ...
    sql = """select valor from parametro_integrador where descricao like 'auditoriaPdv' """
...
def job_updateTabela_minhaGestaoReposicao(data_inicio=None, data_fim=None):
    ...
    conn_business5 = mysql.connector.connect(host="", user="", password="", database="")
    cursorSQLBusiness5 = conn_business5.cursor()

    #connection5 = oracledb.connect(user="", password="", dsn="")
    #cursorOracle5 = connection5.cursor()
        
    sql = """select valor from parametro_integrador where descricao like 'Reposição' """
...
def job_updateTabela_minhaGestaoAtividade(data_inicio=None, data_fim=None):
    ...
    conn_business6 = mysql.connector.connect(host="", user="", password="", database="")
    ...
    sql = """select valor from parametro_integrador where descricao like 'Atividade' """
...
def job_updateTabela_PerdasQuebras(data_inicio=None, data_fim=None):
    ...
    conn_business7 = mysql.connector.connect(host="", user="", password="", database="")
    cursorSQLBusiness7 = conn_business7.cursor()

    connection7 = oracledb.connect(user="", password="", dsn="")
    cursorOracle7 = connection7.cursor()

    sql = """select valor from parametro_integrador where descricao like 'PerdasQuebras' """
    cursorSQLBusiness.execute(sql)
    result_sql = cursorSQLBusiness.fetchall()
    result_sql = str(result_sql).replace("[(", "").replace(",)]", "").replace("'", "")
    parametro_integrador = (result_sql)
...
def job_updateTabela_PedidosAprovadosReprovados(data_inicio=None, data_fim=None):
    ...
    conn_business8 = mysql.connector.connect(host="", user="", password="", database="")
    ...
    sql = """select valor from parametro_integrador where descricao like 'PedidosAprovadosReprovados' """
...
def job_updateTabela_ProdutosEstoqueAtual(data_inicio=None, data_fim=None):
    ...
    conn_business9 = mysql.connector.connect(host="", user="", password="", database="")
    cursorSQLBusiness9 = conn_business9.cursor()

    connection9 = oracledb.connect(user="", password="", dsn="")
    cursorOracle9 = connection9.cursor()

    sql = """select valor from parametro_integrador where descricao like 'ProdutosEstoqueAtual' """
    cursorSQLBusiness.execute(sql)
    result_sql = cursorSQLBusiness.fetchall()
    result_sql = str(result_sql).replace("[(", "").replace(",)]", "").replace("'", "")
    parametro_integrador = (result_sql)
...
def job_updateTabela_vendas_dia_lojas_simplificado(data_inicio=None, data_fim=None):
    ...
    conn_business10 = mysql.connector.connect(host="", user="", password="", database="")
    ...
    sql = """select valor from parametro_integrador where descricao like 'VendaSimplificada' """
...
def job_updateTabela_produtos_por_hora(data_inicio=None, data_fim=None):
    ...
    conn_business11 = mysql.connector.connect(host="", user="", password="", database="")
    ...
    sql = """select valor from parametro_integrador where descricao like 'produtoPorHora' """
    cursorSQLBusiness.execute(sql)
    result_sql = cursorSQLBusiness.fetchall()
    result_sql = str(result_sql).replace("[(", "").replace(",)]", "").replace("'", "")
    parametro_integrador = (result_sql)
...
class InterfaceApp:
    ...
    def carregar_parametros_integrador(self):
        try:
            cursorSQLBusiness.execute("SELECT id, descricao, valor FROM parametro_integrador")
            resultados = cursorSQLBusiness.fetchall()
            
            print("Carregando parâmetros do integrador...")
            for id, descricao, valor in resultados:
                if descricao in self.vars_toggle:
                    self.vars_toggle[descricao].set(int(valor))
                    
                    if descricao in self.executar_buttons:
                        if int(valor) == 1:
                            self.executar_buttons[descricao].pack(side='right', padx=5)
                        else:
                            self.executar_buttons[descricao].pack_forget()
                    
                    print(f"✅ Parâmetro carregado: {descricao} = {valor}")
                else:
                    print(f"⚠️ Parâmetro não encontrado na interface: {descricao}")
    ...
    def carregar_parametros_agendador(self):
        try:
            ...
            cursorSQLBusiness.execute("""
                SELECT descricao, tipo_agendamento, valor 
                FROM parametro_agendador 
            """)
            resultados = cursorSQLBusiness.fetchall()
            ...
            for descricao, tipo_agendamento, valor in resultados:
                agendamento = f"{tipo_agendamento}: {valor}"

                # Atualizar interface
                if descricao in self.horarios:
                    listbox = self.horarios[descricao]
                    listbox.insert(tk.END, agendamento)
                    self.agendamentos_salvos[descricao].append(agendamento)
                else:
                    print(f"Descrição '{descricao}' não encontrada em self.horarios")

                # Agendar execução
                if descricao in self.funcoes_jobs:
                    funcao = self.funcoes_jobs[descricao]
                    try:
                        if tipo_agendamento == "A Cada Hora":
                            try:
                                horas = int(valor.split(':')[0])  # Pega a parte das horas
                                minutos = int(valor.split(':')[1]) if len(valor.split(':')) > 1 else 0
                                segundos = int(valor.split(':')[2]) if len(valor.split(':')) > 2 else 0
                                
                                intervalo = horas + minutos/60 + segundos/3600
                                if intervalo <= 0:
                                    print(f"⚠️ Intervalo inválido para '{descricao}': {valor}")
                                    continue
                                    
                                job = schedule.every(intervalo).hours.do(funcao)
                                self.jobs_agendados[agendamento] = job
                                print(f"⏱️ Agendado '{descricao}' para rodar a cada {intervalo} horas.")
                            except ValueError as e:
                                print(f"⚠️ Valor inválido para 'A Cada Hora' em '{descricao}': {valor} - {e}")
                                
                        elif tipo_agendamento == "Horário Fixo":
                            try:
                                # Verifica se o valor está no formato HH:MM ou HH:MM:SS
                                if re.match(r'^\d{1,2}:\d{2}(:\d{2})?$', valor):
                                    ...
                                    if hora < 0 or hora > 23 or minuto < 0 or minuto > 59 or segundo < 0 or segundo > 59:
                                        print(f"⚠️ Horário inválido para '{descricao}': {valor}")
                                        continue
                                        
                                    job = schedule.every().day.at(valor).do(funcao)
                                    self.jobs_agendados[agendamento] = job
                                    print(f"⏰ Agendado '{descricao}' para rodar todos os dias às {valor}.")
                                else:
                                    print(f"⚠️ Formato de horário inválido para '{descricao}': {valor}")
                            except ValueError as e:
                                print(f"⚠️ Erro ao processar horário fixo '{descricao}': {e}")
                        else:
                            print(f"❌ Tipo de agendamento desconhecido para '{descricao}': {tipo_agendamento}")
                    except Exception as e:
                        print(f"⚠️ Erro ao agendar '{descricao}': {e}")
                else:
                    print(f"⚠️ Função '{descricao}' não encontrada em self.funcoes_jobs")

        except mysql.connector.Error as err:
            print(f"Erro ao carregar agendamentos do banco de dados: {err}")
            
    def salvar_parametro_integrador(self, descricao, valor):
        try:
            cursorSQLBusiness.execute("SELECT id FROM parametro_integrador WHERE descricao = %s", (descricao,))
            existe = cursorSQLBusiness.fetchone()
            
            if existe:
                cursorSQLBusiness.execute(
                    "UPDATE parametro_integrador SET valor = %s WHERE descricao = %s",
                    (valor, descricao))
            else:
                cursorSQLBusiness.execute(
                    "INSERT INTO parametro_integrador (descricao, valor) VALUES (%s, %s)",
                    (descricao, valor))
            
            conn_business.commit()
            print(f"✅ Parâmetro {descricao} salvo com valor {valor}")
        except Exception as e:
            print(f"⚠️ Erro ao salvar parâmetro {descricao}: {e}")

    def salvar_parametro_agendador(self, descricao, tipo, valor):
        try:
            cursorSQLBusiness.execute(
                "INSERT INTO parametro_agendador (descricao, tipo_agendamento, valor) VALUES (%s, %s, %s)",
                (descricao, tipo, valor))
            conn_business.commit()
            print(f"✅ Agendamento salvo: {descricao} - {tipo} - {valor}")
    ...
    def deletar_parametro_agendador(self, descricao, agendamento_str):
        try:
            ...
            if len(partes) == 2:
                ...
                cursorSQLBusiness.execute(
                    "DELETE FROM parametro_agendador WHERE descricao = %s AND tipo_agendamento = %s AND valor = %s",
                    (descricao, tipo, valor))
                conn_business.commit()
                ...
                print(f"✅ Agendamento removido: {descricao} - {tipo} - {valor}")
                return True
            return False
        except Exception as e:
    ...
    def deletar_agendamento(self, nome_var, listbox):
        ...
        if self.deletar_parametro_agendador(nome_var, agendamento):
    ...
    def adicionar_agendamento(self, nome_var, combo, entry):
        ...
        if tipo == "Horário Fixo":
            if not re.match(r'^\d{1,2}:\d{2}(:\d{2})?$', valor):
            ...
            try:
                ...
                if hora < 0 or hora > 23 or minuto < 0 or minuto > 59 or segundo < 0 or segundo > 59:
                    messagebox.showwarning("Horário inválido", "Valores fora do intervalo permitido (Hora: 0-23, Minuto/Segundo: 0-59)")
                    return
            except ValueError:
                messagebox.showwarning("Valor inválido", "Por favor, insira números válidos para hora, minuto e segundo")
                return
            ...
            try:
                ...
                messagebox.showwarning("Valor inválido", "Por favor, insira números válidos para horas, minutos e segundos")
                return

        agendamento = f"{tipo}: {valor}"
        self.horarios[nome_var].insert(tk.END, agendamento)
        self.agendamentos_salvos[nome_var].append(agendamento)
        self.salvar_parametro_agendador(nome_var, tipo, valor)
        print(f"✅ Agendamento Adicionado: {agendamento}")

    def toggle_variavel(self, nome_var, var_bool):
        valor = var_bool.get()
        self.salvar_parametro_integrador(nome_var, valor)
